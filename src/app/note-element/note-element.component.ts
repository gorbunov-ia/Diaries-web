...
export class NoteElementComponent implements OnInit {
  ...
  private updateNoteElement(element: NoteElement): void {
    for (const noteElement of this.notesElements) {
      if (element.sortBy === noteElement.sortBy) {
        noteElement.id = element.id;
        noteElement.description = element.description;
        noteElement.lastModified = element.lastModified;
        return;
      }
    }
  }
